...
class Player {
  ...
  getCurrentAnimationFrame() {
    ...
    } else if ((this.state.falling && this.state.right) || (this.state.falling && this.state.dx > 0)) {
      return this.animations.fall_right.getCurrentFrame();

    } else if ((this.state.falling && this.state.left) || (this.state.falling && this.state.dx < 0)) {
      return this.animations.fall_left.getCurrentFrame();

    } else if (this.state.falling && !this.state.jumping && !this.state.left && !this.state.right) {
  ...
  render(ctx, dt) {
    ctx.drawImage(this.getCurrentAnimationFrame(), this.state.x - 48, this.state.y - 60, GameUtil.TILE * 4, GameUtil.TILE * 3);
