...
class Player {
  ...
  getCurrentAnimationFrame() {
    if ((this.state.jumping) && (this.state.right) || (this.state.jumping) && (this.state.wasRight)) {
      return this.animations.jump_right.getCurrentFrame();
    } else if ((this.state.jumping) && (this.state.left) || (this.state.jumping) && (this.state.wasLeft)) {
  ...
  render(ctx, dt) {
    ctx.drawImage(this.getCurrentAnimationFrame(), this.state.x, this.state.y - 60, GameUtil.TILE * 3, GameUtil.TILE * 3);
